    ...
    try {
        ...
        const processItem = (item) => {
             ...
             if (price > 0 && shares > 0) {
                 ...
                 // 🔍 DEBUG: İlk 3 ticker için log
                 if (Math.random() < 0.01) {
                     console.log(`[MC DEBUG] ${ticker}: price=${price}, shares=${shares}, MC=${mc.toLocaleString()}`);
                 }
                 ...
                 if (Math.random() < 0.005) {
                     console.warn(`[MC WARN] ${ticker}: price=${price}, shares=${shares} - MC hesaplanamadı`);
                 }
             }
        };
